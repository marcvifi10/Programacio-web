<?php
    ...
    $sqlUpdate = "UPDATE `versionjuego` 
                  SET `idJuego`=$nombre,`idPlataforma`=$ptl,`idEdicion`=$edicion,`idDistribuidora`=$dis,`precio`=$precio,`stock`=$stock,`fechaSalida`='$fecha' WHERE idVersion=$idVersion";
    echo $sqlUpdate;
    $bd->query($sqlUpdate);
	$bd->close();
?>
